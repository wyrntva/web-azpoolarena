...
class QRGeneratorWindow(QMainWindow):
    ...
    def generate_qr(self):
        ...
        try:
            ...
            # --- TIMEZONE & CLOCK SKEW LOGIC ---
            # 1. Get expiration time from backend
            expires_str = response["expires_at"]
            
            # Helper to parse ISO format robustly
            if "+" not in expires_str and expires_str[-1] != "Z":
                # Naive time (likely UTC from backend) -> Assume UTC
            ...
            # We record the exact moment we received the response (local monotonic time)
            # This allows us to count seconds accurately regardless of wall-clock changes
            import time
            self.local_start_time = time.monotonic()
            self.token_ttl_seconds = ttl
            
            # Check server time if available to warn about major discrepancies
            server_time = response.get("server_time") # Aware datetime (UTC)
            if server_time:
                # Calculate effective remaining time based on server's perspective
                # remaining = expires_at - server_time
                server_remaining = (self.expires_at - server_time).total_seconds()
                print(f"[DEBUG] Server Time: {server_time}")
                print(f"[DEBUG] Expires At:  {self.expires_at}")
                print(f"[DEBUG] Valid for:   {server_remaining} seconds")
                
                # If backend says token is already expired or valid for too long, adjust
                if abs(server_remaining - ttl) > 5:
                    print(f"[WARN] Large time discrepancy detected! Server thinks token valid for {server_remaining}s")
                    # Trust the TTL over the timestamps if they look weird
                    self.token_ttl_seconds = min(ttl, max(0, int(server_remaining)))
            ...
            self.countdown_timer.start(100) # Update every 100ms for smoothness

        except Exception as e:
            msg = str(e)
            if "401" in msg or "Invalid API key" in msg:
                 msg += "\n\nLỗi xác thực: Vui lòng kiểm tra API Key trong .env\nhoặc khởi động lại Backend Server."
    ...
    def update_countdown(self):
        """Update countdown timer using robust monotonic time"""
        if not hasattr(self, 'local_start_time'):
            return
            
        import time
        # Calculate seconds elapsed since token creation (independent of system clock changes)
        elapsed = time.monotonic() - self.local_start_time
        
        # Calculate remaining time
        remaining = self.token_ttl_seconds - elapsed
        ...
        if remaining <= 0:
            ...
            # Update countdown
            seconds_remaining = int(remaining) + 1 # Ceiling for display
