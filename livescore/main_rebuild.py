...
class ScoreboardOverlay(QWidget):
    ...
    def init_ui(self):
        ...
        self.setWindowTitle("OBS Live Scoreboard Overlay")
...
class SettingsDialog(QDialog):
    ...
    def update_config(self):
        ...
        c["style_1"] = getattr(self, "combo_style_1", self.combo_style_1).currentText() if hasattr(self, "combo_style_1") else "Ngang cao cấp"
        c["style_2"] = getattr(self, "combo_style_2", self.combo_style_2).currentText() if hasattr(self, "combo_style_2") else "Không hiển thị"
...
class ControlPanel(QMainWindow):
    ...
    def setup_hotkeys(self):
        ...
        def toggle_active():
            if self.radio_p1.isChecked(): self.radio_p2.setChecked(True)
            else: self.radio_p1.setChecked(True)
    ...
    def init_ui(self):
        ...
        # Nút +/- điểm nhanh
        btn_p1_up = QPushButton("+ ĐIỂM")
        btn_p1_up.setStyleSheet("background-color: #27ae60; color: white; font-size: 16px; font-weight: bold; padding: 10px; border-radius: 5px;")
        btn_p1_up.clicked.connect(lambda: self.spin_p1_score.setValue(self.spin_p1_score.value() + 1))
        
        btn_p1_down = QPushButton("- ĐIỂM")
        btn_p1_down.setStyleSheet("background-color: #e74c3c; color: white; font-size: 16px; font-weight: bold; padding: 10px; border-radius: 5px;")
        ...
        # Nút +/- điểm nhanh
        btn_p2_up = QPushButton("+ ĐIỂM")
        btn_p2_up.setStyleSheet("background-color: #27ae60; color: white; font-size: 16px; font-weight: bold; padding: 10px; border-radius: 5px;")
        btn_p2_up.clicked.connect(lambda: self.spin_p2_score.setValue(self.spin_p2_score.value() + 1))
        
        btn_p2_down = QPushButton("- ĐIỂM")
        btn_p2_down.setStyleSheet("background-color: #e74c3c; color: white; font-size: 16px; font-weight: bold; padding: 10px; border-radius: 5px;")
        ...
        # ---------------- NÚT SWAP GIỮA 2 NGƯỜI CHƠI ----------------
        btn_swap = QPushButton("🔄")
        btn_swap.setToolTip("Đổi chỗ P1 <-> P2")
        btn_swap.setStyleSheet("font-size: 28px; background-color: transparent; border: none; padding: 10px;")
        btn_swap.setCursor(Qt.CursorShape.PointingHandCursor)
        btn_swap.clicked.connect(self.swap_players)
        grid.addWidget(btn_swap, 0, 1, alignment=Qt.AlignmentFlag.AlignCenter)
        
        grid.addWidget(self.group_p2, 0, 2)

        main_layout.addLayout(grid)

        # ---------------- TRẠNG THÁI LƯỢT CHƠI (ACTIVE PLAYER) ----------------
        self.group_active = QGroupBox("Người đang cầm cơ / Có lượt")
        layout_active = QHBoxLayout()
        self.btn_group_active = QButtonGroup()
        
        self.radio_p1 = QRadioButton("Người chơi 1")
        self.radio_p2 = QRadioButton("Người chơi 2")
        self.radio_none = QRadioButton("Không ai cả")
        
        self.radio_p1.setChecked(True)
        
        self.btn_group_active.addButton(self.radio_p1, 1)
        self.btn_group_active.addButton(self.radio_p2, 2)
        self.btn_group_active.addButton(self.radio_none, 0)
        
        self.btn_group_active.idToggled.connect(self.update_active_player)
        
        layout_active.addWidget(self.radio_p1)
        layout_active.addWidget(self.radio_p2)
        layout_active.addWidget(self.radio_none)
        
        self.group_active.setLayout(layout_active)
        main_layout.addWidget(self.group_active)
        ...
        # Dòng ghi chú kéo thả
        lbl_tip = QLabel("Mẹo: Bạn có thể giữ chuột trái trên Bảng điểm và kéo thả để di chuyển bảng điểm đến vị trí mong muốn trên màn hình.")
        lbl_tip.setWordWrap(True)
        lbl_tip.setStyleSheet("font-style: italic; color: #7f8c8d; margin-top: 5px;")
    ...
    def swap_players(self):
        ...
        if self.radio_p1.isChecked():
            self.radio_p2.setChecked(True)
        elif self.radio_p2.isChecked():
            self.radio_p1.setChecked(True)
            
        self.update_data()

    def update_active_player(self, player_id, checked):
        if checked:
    ...
    def update_borderless(self):
        self.config["borderless"] = self.chk_borderless.isChecked()
        if hasattr(self, 'overlay1') and self.overlay1:
