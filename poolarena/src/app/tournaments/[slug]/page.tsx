...
export default function TournamentDetailPage() {
  ...
  const isFull = tournament && !tournament._isEvent ? players.length >= tournament.participants.max : false;
  ...
  const fetchTournament = async () => {
    ...
    try {
      ...
      // If this item is an event, redirect to /events/:slug
      if (data.category === 'event') {
        router.replace(`/events/${data.slug || tournamentSlug}`);
        return;
      }
      ...
      const formatted: TournamentDetail = {
        id: data.id.toString(),
        title: data.name || (isEvent ? 'SỰ KIỆN ARENA POOL' : 'GIẢI ĐẤU ARENA POOL'),
      ...
      <div className="block sm:hidden bg-[#F0F2F4]">
        ...
        <div className="px-4 -mt-[70px] pb-8 relative z-10 flex flex-col gap-5">
          ...
          <div className="bg-[#172339] text-white rounded-2xl p-4 w-full h-auto flex flex-col justify-between shadow-[0_10px_30px_rgba(23,35,57,0.15)] relative overflow-hidden -mt-[36px] pb-6">
            ...
            {/* First Prize */}
            <div className="flex flex-col py-3 px-4 w-full">
              <span className="text-[16px] text-[#BAE3FF] font-normal leading-[24px]" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                {tournament._isEvent ? 'Top 1' : 'Vô địch'}
              </span>
            ...
            <div className="flex flex-col py-3 px-4 w-full">
              <span className="text-[16px] text-[#BAE3FF] font-normal leading-[24px]" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                {tournament._isEvent ? 'Top 2' : 'Giải nhì'}
              </span>
              <span className="text-[20px] font-extrabold italic text-white leading-[28px] mt-1" style={{ fontFamily: 'Montserrat, sans-serif' }}>
            ...
            <div className="flex flex-col py-3 px-4 w-full">
              <span className="text-[16px] text-[#BAE3FF] font-normal leading-[24px]" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                {tournament._isEvent ? 'Top 3' : 'Đồng giải ba'}
              </span>
              <span className="text-[20px] font-extrabold italic text-white leading-[28px] mt-1" style={{ fontFamily: 'Montserrat, sans-serif' }}>
            ...
            <button
              ...
              disabled={isAlreadyRegistered || tournament.canRegister === false || (!tournament._isEvent && tournament.participants.current >= tournament.participants.max)}
              className={`w-full text-white font-medium text-[16px] leading-[24px] h-[40px] py-2 px-6 rounded-full flex items-center justify-center transition-all duration-300 shadow-md ${
                isAlreadyRegistered
                  ...
                  : (tournament.canRegister === false || (!tournament._isEvent && tournament.participants.current >= tournament.participants.max))
            ...
            >
              {isAlreadyRegistered
                ...
                : tournament.canRegister === false
                ? "Khóa đăng ký"
                : (!tournament._isEvent && tournament.participants.current >= tournament.participants.max)
                ? "Đã đầy"
                : "Đăng ký ngay"}
            </button>
          </div>
