...
export default function EventsPage() {
  ...
  const fetchTournaments = useCallback(async () => {
    try {
      setLoading(true);
      const response = await tournamentAPI.getTournaments({ category: 'event' });
  ...
  const handleRegister = useCallback((tournamentId: number) => {
    ...
    if (tournament) {
      router.push(`/events/${tournament.slug || tournament.id}`);
    }
    ...
    <div className="min-h-screen bg-[#F0F2F4]">
      ...
      <main className="max-w-[1360px] mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 2xl:px-0 pt-4 sm:pt-6 pb-[100px]">
        <h1 className="sr-only">
          Sự kiện bida Poolarena VietNam
        </h1>
        ...
        {loading ? (
          ...
          <>
            {/* Events Banner — mobile: 361×74 ratio, scales up on larger screens */}
            {bannerUrls.length > 0 && (
              <div
            ...
            <div className="space-y-6 sm:space-y-12">
              ...
              <div className="space-y-4 sm:space-y-6">
                ...
                {upcomingTournaments.length === 0 ? (
                  <div className="text-center py-8 sm:py-12 text-gray-500 text-sm sm:text-base">Chưa có sự kiện sắp diễn ra</div>
                ) : (
                  <TournamentList
              ...
              <div className="space-y-4 sm:space-y-6">
                ...
                {completedTournaments.length === 0 ? (
                  <div className="text-center py-8 sm:py-12 text-gray-500 text-sm sm:text-base">Chưa có sự kiện đã kết thúc</div>
                ) : (
                  <TournamentList
