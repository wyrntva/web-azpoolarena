...
export async function generateMetadata({
  ...
  if (!data) {
    return {
      title: 'Sự kiện bida',
      description: 'Xem thông tin chi tiết về sự kiện bida tại Poolarena VietNam',
    };
  }

  const title = data.name;
  const location = data.location || 'AZ Pool Arena';
  const description = `Sự kiện bida ${data.name} tại ${location}. Đăng ký tham gia ngay tại Poolarena VietNam!`;
...
export default async function EventSlugLayout({
  ...
  const jsonLd = data
    ? {
        ...
        description: `Sự kiện bida ${data.name} tại ${data.location || 'AZ Pool Arena'}.`,
        location: {
          '@type': 'Place',
  ...
  const eventName = data?.name || 'Sự kiện';
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: eventName, item: `${SITE_URL}/events/${slug}` },
    ],
  };
