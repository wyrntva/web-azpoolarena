...
export function HomeScreen({ navigation }: Props) {
  const { member, notifications } = useAuth();

  const shortcutItems = shortcuts.map((item) => ({
    ...item,
    onPress: () => {
      if (item.key === "delivery") navigation.navigate("Booking");
      if (item.key === "store") navigation.navigate("Profile");
      if (item.key === "transaction") navigation.navigate("MemberHistory");
      if (item.key === "gift") navigation.navigate("Notification");
    },
  }));
  ...
  return (
    <View style={styles.screen}>
      ...
      >
        <HomeHeader
          memberName={member?.name ?? "Việt Anh Trần"}
          rankLabel={member?.rankLabel ?? "Thành viên Bạc"}
        ...
        <ShortcutMenu items={shortcutItems} />

        <View style={styles.sectionBlock}>
          <SectionHeader icon="🎁" title="Ưu đãi đỉnh cao" />
        ...
        <View style={styles.sectionBlockTight}>
          <SectionHeader
            ...
            onPressAction={() => navigation.navigate("Notification")}
          />
      ...
      <HomeBottomNav
        items={bottomItems}
        onPress={(key) => {
          if (key === "offers") navigation.navigate("Notification");
          if (key === "scan") navigation.navigate("MemberCard");
          if (key === "stamp") navigation.navigate("Booking");
          if (key === "hot") navigation.navigate("Tournament");
        }}
      />
    </View>
  );
