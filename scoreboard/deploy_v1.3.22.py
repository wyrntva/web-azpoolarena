#!/usr/bin/env python3
"""
Deploy v1.3.22 — Bugfix: Dynamic check-in updates and auto-closing join dialog

Files deployed:
  - qml/pages/TournamentPage.qml: Update onMatchChanged to handle check-in updates dynamically
  - qml/components/TournamentJoinDialog.qml: Add left/right confirmed property listeners to auto-check
  - VERSION: 1.3.21 → 1.3.22

Usage: python deploy_v1.3.22.py
"""
...
# Ensure stdout handles UTF-8 output if any, but we will print pure ASCII to be safe.
...
def main():
    version = file_contents.get("VERSION", "1.3.22")
    print("=" * 64)
    print(f"  AZ Scoreboard - Deploy v{version}")
    print("  Bugfix: Dynamic check-in and auto-closing join dialog")
