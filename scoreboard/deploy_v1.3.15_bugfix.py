#!/usr/bin/env python3
"""
Deploy v1.3.15 — Bugfix: score không bị nhảy xuống khi ấn tỉ số

Các thay đổi:
  - qml/pages/TournamentPage.qml: sửa điều kiện playersChanged để tránh
    reset điểm sai khi player thay đổi từ TBD (null) → ID thật
  - core/tournament_service.py: không có thay đổi (file vẫn được push để
    đảm bảo version đồng bộ)
  - VERSION: 1.3.14 → 1.3.15

Chạy: python3 deploy_v1.3.15_bugfix.py
"""
...
def update_machine(ip):
    ...
    try:
        ...
        # 1. Tạo thư mục tạm
        ssh.exec_command("mkdir -p /tmp/sb_up/core /tmp/sb_up/qml/pages")[1].channel.recv_exit_status()
        ...
        for rel_path, content in file_contents.items():
            if rel_path == "VERSION":
                remote_tmp = f"/tmp/sb_up/VERSION"
            else:
...
def main():
    version = file_contents.get("VERSION", "1.3.15")
    print(f"{'='*64}")
    print(f"  AZ Scoreboard — Deploy Bugfix v{version}")
    print(f"  Fix: score không bị nhảy xuống khi ấn tỉ số")
