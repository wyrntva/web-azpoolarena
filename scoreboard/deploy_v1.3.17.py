#!/usr/bin/env python3
"""
Deploy v1.3.17 — Bugfix: score jump-back khi bấm nhanh

Các thay đổi (3 files):
  - qml/components/ScoreTile.qml: debounce 400ms chống ghost touch
  - qml/pages/TournamentPage.qml: fix Math.max + null check thay || falsy
  - core/tournament_service.py: không gọi fetchActiveMatch sau mỗi PUT score
    (chỉ gọi khi có winner). Tránh race condition ghi đè score local.
  - VERSION: 1.3.16 → 1.3.17

Chạy: python3 deploy_v1.3.17.py
"""
...
def update_machine(ip):
    ...
    try:
        ...
        ssh.exec_command(
            "mkdir -p /tmp/sb_up/qml/components /tmp/sb_up/qml/pages /tmp/sb_up/core"
...
def main():
    version = file_contents.get("VERSION", "1.3.17")
    print(f"{'='*64}")
    print(f"  AZ Scoreboard — Deploy v{version}")
    print(f"  Fix: ghost touch debounce + score race condition")
