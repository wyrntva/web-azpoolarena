#!/usr/bin/env python3
"""
Deploy v1.3.33 — Event dialog styling, event icon, UI improvements
...
Files deployed:
  ...
  - assets/icon/event_icon.svg
  - all core, qml, assets, scripts

Target: 192.168.1.195
Usage: python deploy_v1.3.33.py
"""
...
def update_machine(ip: str, archive_data: bytes):
    ...
    try:
        ...
        _, out, _ = ssh.exec_command(
            f"echo '{PASSWORD}' | sudo -S pkill -f 'python.*app.py'; sleep 2; echo restarted"
        )
        out.read()
        ...
        # Verify running PID
        _, out, _ = ssh.exec_command("pgrep -f 'python.*app.py' || echo 'not running yet'")
...
def main():
    print("=" * 64)
    print(f"  AZ Scoreboard — Full Deploy v1.3.33")
    print(f"  Event dialog updates, event icon, UI fixes")
