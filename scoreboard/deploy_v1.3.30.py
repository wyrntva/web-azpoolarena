#!/usr/bin/env python3
"""
Deploy v1.3.30 — Full deployment of MQTT support and scoreboard updates

Files deployed:
  - app.py                         : exposes MqttService to QML
  - core/live_score_service.py     : handles score changes and API requests
  - core/mqtt_service.py           : handles MQTT connections, reset_scores + reset_match signals
  - qml/Main.qml                   : QML connections for MqttService
  - qml/pages/ScorePage.qml        : onResetScoresRequested / onResetMatchRequested handlers
  - qml/pages/MultiScorePage.qml   : multiplayer reset handlers + sync
  - qml/pages/MultiCardScorePage.qml : same as above
  - qml/pages/MultiQuickAddPage.qml  : same as above
  - requirements.txt               : paho-mqtt dependency
  - VERSION                        : 1.3.30

Usage: SB_PASS=admin python deploy_v1.3.30.py
"""
...
def update_machine(ip: str):
    ...
    try:
        ...
        lines.append("+ Installed pip requirements (paho-mqtt)")
...
def main():
    version = file_contents.get("VERSION", "1.3.30")
    print("=" * 64)
    print(f"  AZ Scoreboard — Deploy v{version}")
    print("  Full deployment with MQTT dependencies and files")
