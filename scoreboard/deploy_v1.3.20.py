#!/usr/bin/env python3
"""
Deploy v1.3.20 — Features: Table fee payment dialog, auto-restart timer updates, absent detection

Các thay đổi:
  - core/tournament_service.py: Request/cancel/check table fee payments on backend
  - qml/components/TournamentJoinDialog.qml: Handle absent detection signal
  - qml/components/TableFeePaymentDialog.qml: Show QR and poll payment status
  - qml/pages/TournamentPage.qml: Integrate table fee payment dialog and match timer controls
  - VERSION: 1.3.19 → 1.3.20

Chạy: python deploy_v1.3.20.py
"""
...
def main():
    version = file_contents.get("VERSION", "1.3.20")
    print(f"{'='*64}")
    print(f"  AZ Scoreboard - Deploy v{version}")
    print(f"  Features: Table fee payment dialog, timer updates, absent detection")
