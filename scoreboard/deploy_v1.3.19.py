#!/usr/bin/env python3
"""
Deploy v1.3.19 — Fix: scoreboard hiển thị đúng race-to cho vòng loại trực tiếp

Root cause:
  Backend trả effective_race_to (8) cho chung kết / bán kết nhưng TournamentPage.qml
  chưa đọc field này → vẫn dùng draw_touch (7) → hiển thị "Chạm 7" thay vì "Chạm 8".

Các thay đổi:
  - qml/pages/TournamentPage.qml:
      + updateMatchRules(): return sớm khi có effective_race_to (flat target, no handicap)
        → dùng giá trị final/semi_final/quarter_final từ backend
  - VERSION: 1.3.18 → 1.3.19

Chạy: python3 deploy_v1.3.19.py
"""
...
def main():
    version = file_contents.get("VERSION", "1.3.19")
    print(f"{'='*64}")
    print(f"  AZ Scoreboard — Deploy v{version}")
    print(f"  Fix: effective_race_to cho chung kết / bán kết")
