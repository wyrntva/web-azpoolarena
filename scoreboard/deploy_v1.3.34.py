#!/usr/bin/env python3
"""
Deploy v1.3.34 — Event dialog custom race/handicap, race >= 9 rule, 45min rule, numpad layout fix

Files deployed:
  - VERSION                         : 1.3.34
  - qml/Main.qml                    : Fixed numpad height & transform origin
  - qml/components/DialogShell.qml  : Dynamic hole cutout bám sát numpad
  - qml/components/EventMatchDialog.qml : Custom race to >= 9, handicap selection & live calculation
  - qml/pages/TournamentPage.qml    : 45 min duration rule & Amber warning for event matches
  - all core, qml, assets, scripts

Target: 192.168.1.195 (or passed via CLI)
Usage: python deploy_v1.3.34.py [IP1 IP2 ...]
"""
...
def main():
    ...
    print("=" * 64)
    print(f"  AZ Scoreboard — Deploy v1.3.34")
    print(f"  Event custom race/handicap, 45m rule, numpad layout fix")
