...
export default function LevelTypeSection({
    formData, setFormData, ranks, handleRankToggle, isEvent = false,
        ...
        <div className="bg-gray-50 dark:bg-gray-800 p-6 rounded-lg space-y-4">
            ...
            <div>
                <Label className="mb-3 block text-blue-600 dark:text-blue-400">
                    ...
                    {isEvent && (
                        <span className="ml-2 text-xs text-gray-500 font-normal">(Tối đa 2 level)</span>
                    )}
                </Label>
                <div className="flex flex-wrap gap-3">
                    {ranks.map((rank) => {
                        const isChecked = formData.ranks.includes(rank.name);
                        const maxReached = isEvent && formData.ranks.length >= 2 && !isChecked;
                        return (
                            <div key={rank.id} className={`flex items-center gap-2 ${maxReached ? 'opacity-40' : ''}`}>
                                <Checkbox
                                    id={`rank-${rank.id}`}
                                    checked={isChecked}
                                    onChange={() => !maxReached && handleRankToggle(rank.name)}
                                    disabled={maxReached}
                                />
                                <Label htmlFor={`rank-${rank.id}`} className={`font-medium ${maxReached ? 'cursor-not-allowed' : 'cursor-pointer'}`}>
