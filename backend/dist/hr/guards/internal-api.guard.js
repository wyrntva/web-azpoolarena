...
let InternalApiGuard = class InternalApiGuard {
    ...
    canActivate(context) {
        ...
        if (!internalKey) {
            throw new common_1.InternalServerErrorException('Internal API key not configured on server');
        }
        const requestKey = request.headers['x-internal-api-key'];
        if (!requestKey) {
            throw new common_1.UnauthorizedException('Missing X-Internal-API-Key header');
        }
